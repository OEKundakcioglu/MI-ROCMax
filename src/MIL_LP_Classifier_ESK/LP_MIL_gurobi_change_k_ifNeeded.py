...
results=pd.DataFrame(columns = colnames,  index = np.arange(0,(A.size*5*10)))
cnt=0
...
for ind in A:
    ...
    labels[labels==0]=-1
    # alldat.iloc[:,2:alldat.shape[1]]=scale(alldat.iloc[:,2:alldat.shape[1]])
    ...
    for rep in range(0, 5):
         for k in range(0, nfold):
            ...
            if dataRepresentation=='R^cluster': #dissimilarity to the cluster centers (R^cluster)
                ...
                currentIter = iter
                while( clusterRange[currentIter] >= traininstances.shape[0]):
                    currentIter = currentIter -1
                    print("CLUSTER DECREASED___ Not enough data points for this number of clusters")
                    nofClust=clusterRange[currentIter]
    ...
    results = results.append(currentSolutionValues)
    results.to_csv(resultsFolder+'LP_'+ dataRepresentation +'_change_k_ifneeded_' +str(fil[ind]))